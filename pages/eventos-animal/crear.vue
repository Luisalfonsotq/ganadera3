  ...
  <div class="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-xl">
    ...
    <form @submit.prevent="handleCrear" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <!-- Tipo de Evento desde catálogo estático -->
        <div>
          <label class="block text-sm font-medium text-gray-700">Tipo de Evento *</label>
          <select v-model="form.tipo_evento" required
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border bg-white">
            <option :value="null">Selecciona el tipo</option>
            <optgroup v-for="(nombres, categoria) in tiposEventosAnimal" :key="categoria" :label="labelCategoria(categoria)">
              <option v-for="nombre in nombres" :key="nombre" :value="nombre">
                {{ nombre }}
              </option>
            </optgroup>
          </select>
    ...
    <div v-if="success" class="mt-4 p-3 bg-green-100 text-green-700 rounded-md font-medium">✅ {{ success }}</div>
    <div v-if="eventoError" class="mt-4 p-3 bg-red-100 text-red-700 rounded-md font-medium">❌ Error: {{ eventoError }}</div>
  </div>
</template>
...
const form = ref({
  animal_id: null,
  tipo_evento: null,  // nombre del tipo como string
...
const esCambioPotrero = computed(() =>
  form.value.tipo_evento === 'Cambio de potrero'
)
const esBaja = computed(() =>
  (tiposEventosAnimal.bajas || []).includes(form.value.tipo_evento)
)
const esPesaje = computed(() =>
  form.value.tipo_evento === 'Pesaje'
)
...
const loadPotreros = async () => {
  ...
  if (fincasData.value?.length > 0) {
    const { data: potrerosData } = await getPotrerosOfFinca(fincasData.value[0].id)
    if (potrerosData.value) potreros.value = potrerosData.value
  }
  loadingPotreros.value = false
}

watch(user, () => { loadAnimales(); loadPotreros() }, { immediate: true })
onMounted(() => { loadAnimales(); loadPotreros() })
...
const handleCrear = async () => {
  ...
  try {
    if (!form.value.animal_id) throw new Error('Debes seleccionar el animal.')
    if (!form.value.tipo_evento) throw new Error('Debes seleccionar el tipo de evento.')
    if (!form.value.fecha) throw new Error('Debes ingresar la fecha del evento.')
    ...
    if (esBaja.value) {
      if (!confirm(`ADVERTENCIA: Estás a punto de registrar un evento de "${form.value.tipo_evento}".\n\nEsto eliminará al animal del registro activo.\n\n¿Deseas continuar?`)) return
    }

    const nuevoEvento = await createEvento(dataToSend)
    if (nuevoEvento) {
      success.value = 'Evento creado exitosamente.'
      setTimeout(() => router.push('/eventos-animal'), 1500)
    } else {
      eventoError.value = 'Error al crear el evento.'
    }
  } catch (err) {
