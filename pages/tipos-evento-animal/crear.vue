<!-- pages/tipos-evento-animal/crear.vue -->
<template>
  <div class="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-xl">
    <h2 class="text-2xl font-bold mb-6 text-center text-gray-800">Crear Nuevo Tipo de Evento de Animal</h2>
    
    <form @submit.prevent="handleCrear" class="space-y-4">
      <div class="grid grid-cols-1 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700">Nombre *</label>
          <input type="text" v-model="form.nombre" required
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border"
            placeholder="Ej: Nacimiento, Baja, Venta, Muerte">
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700">Descripción</label>
          <textarea v-model="form.descripcion" rows="3"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border"
            placeholder="Descripción del tipo de evento"></textarea>
        </div>
      </div>

      <div class="pt-4">
        <button type="submit" :disabled="isSubmitting"
          class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md shadow-md transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
          {{ isSubmitting ? 'Guardando...' : 'Crear Tipo de Evento' }}
        </button>
      </div>
    </form>
    
    <div v-if="success" class="mt-4 p-3 bg-green-100 text-green-700 rounded-md font-medium">
      ✅ {{ success }}
    </div>
    <div v-if="errorMsg" class="mt-4 p-3 bg-red-100 text-red-700 rounded-md font-medium">
      ❌ Error: {{ errorMsg }}
    </div>
  </div>
</template>

<script setup>
definePageMeta({ layout: 'profile-layout' })

const { createTipoEvento } = useTipoEventoAnimal()
const { user } = useAuth()
const router = useRouter()

const form = ref({
  nombre: '',
  descripcion: '',
})

const isSubmitting = ref(false)
const success = ref(null)
const errorMsg = ref(null)

onMounted(() => {
  if (!user.value || user.value.rol !== 'Administrador') {
    alert('No autorizado para crear tipos de evento')
    router.push('/tipos-evento-animal')
  }
})

const handleCrear = async () => {
  errorMsg.value = null
  success.value = null
  isSubmitting.value = true

  try {
    if (!form.value.nombre || !form.value.nombre.trim()) {
      throw new Error('Debes ingresar el nombre del tipo de evento.')
    }

    const dataToSend = {
      nombre: form.value.nombre.trim(),
    }

    if (form.value.descripcion && form.value.descripcion.trim()) {
      dataToSend.descripcion = form.value.descripcion.trim()
    }

    const nuevoTipo = await createTipoEvento(dataToSend)
    if (nuevoTipo) {
      success.value = 'Tipo de evento creado exitosamente.'
      setTimeout(() => {
        router.push('/tipos-evento-animal')
      }, 1500)
    } else {
      errorMsg.value = 'Error al crear el tipo de evento.'
    }
  } catch (err) {
    console.error('Error con más detalles:', err)
    if (err.data?.message) {
      errorMsg.value = err.data.message
    } else {
      errorMsg.value = err.message || 'Error al crear el tipo de evento.'
    }
  } finally {
    isSubmitting.value = false
  }
}
</script>